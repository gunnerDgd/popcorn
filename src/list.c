...
bool_t
    po_list_new
        (po_list* par_list, u32_t par_count, va_list par)                    {
            ...
            if (!po_make_at(&par_list->begin, po_node) from(0)) return false_t;
            if (!po_make_at(&par_list->end  , po_node) from(0)) return false_t;
            if (!po_next_as(&par_list->begin, &par_list->end))  return false_t;
            return true_t;
}
